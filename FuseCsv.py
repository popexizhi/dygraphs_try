...
class fuse_csv():
    def __init__(self, fp1=None, fp2=None):
        ...
        if None == fp1:
            ...
            self.fusefile = "%s_%s.csv" % (self.fp1, self.fp2)
    def save_stafile(self, fp):
        """
        save self.com*_sta 到fp中:w
        """
        assert self.com1_sta
        assert self.com2_sta
...
if __name__=="__main__":
    t = fuse_csv(sys.argv[1], sys.argv[2])
    print t.getres(sys.argv[3], is_tot=None)
    t.save_stafile(sys.argv[4])
